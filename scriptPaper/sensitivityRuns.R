...
start_time <- Sys.time()
run = fitModel(run$dat_l,conf_l, run$confPred,run$dat_alk,run$conf_alk)
end_time <- Sys.time()
...
start_time <- Sys.time()
runDL1 = fitModel(run$dat_l,conf_l, run$confPred,run$dat_alk,run$conf_alk)
end_time <- Sys.time()
timeUsed = end_time - start_time
timeUsed
...
###################################################################################
#No spatial or spatio-temporal effects
load("results/run.Rdata")
conf_l = run$conf_l
conf_alk = run$conf_alk
conf_l$spatioTemporal = 0
...
timeUsed = end_time - start_time
timeUsed
saveIndex(runNoSST,file = "indexJointNoSST.txt", folder = "data/NEAhaddockAssessment/")
save(runNoSST,file = "results/runNoSST.Rdata")
