plotSim = function(sim,...){
  ...
  pl = run$pl
  plSd = run$plSd
  what = c("","log_sigma", "log_kappa","tan_rho_l", "betaLength_alk")
  ...
  namesGreek[which(namesWhat=="tan_rho_l")] = c(expression(rho[alpha][",l"]),expression(rho[gamma][",l"]),expression(rho[psi][",l"]))
  ...
  namesGreek[which(namesWhat=="tan_rho_t")] = c(expression(rho[gamma][",y"]))
  ...
  k = lapply(sim, function(f){
    if(length(f)>1){
      plSim = f$pl
      points(unlist(plSim[what]), col = 'red', pch = 2, cex = 0.5)
    }
  })
}
...
abundanceSim = function(run,sim){
  ...
  for(i in 1:length(sim)){
    ...
    simObj =  fitModel(run$dat_l,run$conf_l, run$confPred,run$dat_alk,run$conf_alk ,parSet = par, runModel = FALSE)
#    rep =simObj$obj$report()
    ...
    abundanceRealizations[[i]] = rep$logAgeIndex
  }
