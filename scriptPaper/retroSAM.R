...
ssbplot(fitWeb, col = 'green',cicol = alpha("green",0.1),xlim = c(2000,2022), main = expression(Proposed~index~with~cov(bold(I)[y]^fixedALK) ),cex.main = 2, ylim =c(0,650000))
ssbplot(fitSepCovRet,xlim = c(2000,2020), add = TRUE)
dev.off()

png("figures/ssbJointCovRet.png")
ssbplot(fitWeb, col = 'green',cicol = alpha("green",0.1),xlim = c(2000,2022), main = expression(Proposed~index~with~cov(bold(I)[y]) ),cex.main = 2, ylim =c(0,650000))
ssbplot(fitJointCovRet,xlim = c(2000,2020), add = TRUE)
dev.off()

png("figures/ssbJointRet.png")
ssbplot(fitWeb, col = 'green',cicol = alpha("green",0.1),xlim = c(2000,2022), main = expression(Proposed~index),cex.main = 2, ylim =c(0,650000))
ssbplot(fitJointRet,xlim = c(2000,2020), add = TRUE)
dev.off()

png("figures/ssbRet.png")
ssbplot(fitWeb, col = 'green',cicol = alpha("green",0.1),xlim = c(2000,2022), main = expression(Official~index~currently~used),cex.main = 2, ylim =c(0,650000))
ssbplot(fitRet,xlim = c(2000,2020), add = TRUE)
dev.off()

png("figures/fRet.png")
fbarplot(fitWeb,partial = FALSE, col = 'green',cicol = alpha("green",0.1),xlim = c(2000,2022), main = expression(Official~index~currently~used),cex.main = 2, ylim =c(0,0.6))
fbarplot(fitRet, add = TRUE)
dev.off()

png("figures/fSepCovRet.png")
fbarplot(fitWeb,partial = FALSE, col = 'green',cicol = alpha("green",0.1),xlim = c(2000,2022), main = expression(Proposed~index~with~cov(bold(I)[y]^fixedALK) ),cex.main = 2, ylim =c(0,0.6))
fbarplot(fitSepCovRet, add = TRUE)
dev.off()

png("figures/fJointCovRet.png")
fbarplot(fitWeb,partial = FALSE, col = 'green',cicol = alpha("green",0.1),xlim = c(2000,2022), main = expression(Proposed~index~with~cov(bold(I)[y]) ),cex.main = 2, ylim =c(0,0.6))
...
for(i in 0:length(fitSepCovRet)){
  ...
  lines( ellipse::ellipse( covJoint, centre = muJoint,level = 0.95) , col="brown",type = 'l',lt = 3,lw = 1)
  points(muJoint[1],muJoint[2],cex = 2,pch = 20, col = 'brown')
  lines( ellipse::ellipse( covJointCov, centre = muJointCov,level = 0.95) , col="green",type = 'l',lw = 1)
  points(muJointCov[1],muJointCov[2],cex = 1,pch = 15, col = 'green')
  ...
  if(i==0){
    legend("bottom", inset=.01,
           ...
           pch = c(19,20,15,17,18), col = c("black", "brown","green","red","blue"), cex = 0.6, lwd = 2, lty = c(2,3,1,1,2))
