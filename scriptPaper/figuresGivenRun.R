...
for(i in 1:dim(parameter)[1]){
  parameter[i,1] = paste0(round(as.numeric(parameter[i,1]),2), " (",round(as.numeric(parameter[i,2]),2), ",",round(as.numeric(parameter[i,3]),2),")")
}
...
pp = xtable(parPrint, type = "latex",digits = 2,lable = NULL)
...
zlim = c(0,max(log(max(aa[years,age-1,])),log(max(ll[years,l,]))))
lab = FALSE
for(year in years){
...
for(year in years){
  ...
  points(attributes(run$data)$locObs[which(attributes(run$data)$year == (year +1993)),][which(AA[,age-1]<1),],pch = 3)
  dev.off()
}
...
#indexSdTmp[indexSdTmp^2>1] = sqrt(1)
maxVar = max(indexSdTmp) + 0.01
nCol = 100
col = heat.colors(nCol+10)

par(oma=c(4,4,3,3),mar=c(0.5,0.5,0.5,0.0))
plot(rep(1, dim(ageLogIndex)[2]),2:run$data$ageRange[2],cex = (ageLogIndex[1,]-0.99*tmp2)/(tmp-tmp2) *increase ,
...
for(i in 2:nYears){
  points(rep(i, dim(ageLogIndex)[2]),2:run$data$ageRange[2],
         cex = (ageLogIndex[i,] -0.99*tmp2)/(tmp-tmp2) *increase,
...
for(year in yIndex){
  ...
  image(x,y, z,col =  colorRampPalette(c("green","yellow", "red"))(length(breaks)-1),
             legend.width = 2, cex = 1.6,zlim = zlim,
             xlab = "East (km)", ylab = "North (km)",
             main = "",
...
breaks = c(-1,seq(zlim[1],zlim[2],by  = 0.05))
image.plot(x,y, z,col =  colorRampPalette(c("green","yellow", "red"))(length(breaks)-1),
           legend.width = 2, cex = 1.6,zlim = zlim,
           xlab = "East (km)", ylab = "North (km)",
           main = "",
           cex.lab = 1.4,cex.main = 1.5)
...
for(year in 1:27){
  ...
  title(1993 + year,line = -2,cex.main=2)
  polygon(mapTmp,col = 'lightgrey')
  points(attributes(run$data)$locObs[which(attributes(run$data)$year == (year +1993)),])
  if(year==27){
    plot(1,1,axes=F,ylim = c(-99,-98))
    image.plot(x,y, z,col =  colorRampPalette(c("white","yellow", "red"))(length(breaks)-1),
               legend.width = 2, cex = 1.6,zlim = zlim,
