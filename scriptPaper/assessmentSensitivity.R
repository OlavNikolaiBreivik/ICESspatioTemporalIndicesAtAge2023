...
###################################################No spatial or spatio-temporal structure#############################
indexJoint= as.matrix(read.table("data/NEAhaddockAssessment/indexJointNoSST.txt", sep = " "))[,-1]
load("data/NEAhaddockAssessment/cov_indexJointNoSST.Rda")
covJoint = lapply(covYears,function(f){
  cov = f[-1,]
  cov = cov[,-1]
  cov
...
conf = loadConf(dat, "data/NEAhaddockAssessment/conf.cfg")
par<-defpar(dat,conf)
fitJointCovNoSST<-sam.fit(dat,conf,par)
