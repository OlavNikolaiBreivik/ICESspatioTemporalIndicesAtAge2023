...
conf_l = defConf(years = 1994:2020, # years to use, use all years with data by default
                 ...
                 minDepth=150,maxDepth=400, #Approximately 80% coverage interval
                 applyALK = 1,
                 cutoff =80, cbound = 160)

#Define configurations age part
conf_alk = defConf_alk(maxAge = 10,
                       minAge = 3,
                       spatioTemporal = 0,
                       spatial =0,readability = 0,
                       rwBeta0 = 1)
...
#which(dat_alk$station %in% tail(unique(dat_alk$station),100))
...
png("figures/figures/withoutRandomQQ_ALK.png")
qqnorm(res$residual,main = "Quantile-quantile with random effects",
       cex.main = 1.5, cex.lab = 1.5, cex = 1.5,cex.axis = 1.5)
abline(0,1)
dev.off()
png("figures/figures/withoutRandomACF_ALK.png")
acf(res$residual,main = " ", ylab = " ",
    cex.main = 2, cex.lab = 1.5, cex = 1.5,cex.axis = 1.5)
title(main="Autocorrelation with random effects",cex.main = 1.5, ylab = "Autocorrelation", cex.lab = 1.5)
dev.off()
